...
def single_view_gallery_evaluation(feature, label, seq_type, view, dataset, metric):
    ...
    msg_mgr.log_info(f"=== Begin single_view_gallery_evaluation for {dataset} ===")

    for (type_, probe_seq) in probe_seq_dict[dataset].items():  # first is nm-03 nm-04
        map[type_] = np.zeros(view_num) - 1.
        acc[type_] = np.zeros((view_num, view_num, num_rank)) - 1.  # acc['nm']  is view_num*view_num matrix

        for (v1, probe_view) in enumerate(view_list):   # 遍历各个 view 
            # probe mask
            ...
            msg_mgr.log_info(f"[Probe] type={type_}, probe_view={probe_view}, probe_seq={probe_seq}, probe_count={len(probe_y)}")
            if len(probe_y) == 0:
                msg_mgr.log_warning(f"⚠️ Probe empty -> type={type_}, view={probe_view}, seq={probe_seq}. Skipping this probe_view.")
                continue
            ...
            for (v2, gallery_view) in enumerate(view_list):
                ...
                msg_mgr.log_info(f"    [Gallery] gallery_view={gallery_view}, gallery_seq={gallery_seq_dict[dataset]}, gallery_count={len(gallery_y)}")

                if len(gallery_y) == 0:
                    msg_mgr.log_warning(f"    ⚠️ Gallery empty -> type={type_}, probe_view={probe_view}, gallery_view={gallery_view}. Skipping this gallery_view.")
                    continue

                if probe_x.shape[1] != gallery_x.shape[1]:
                    msg_mgr.log_error(f"    ⚠️ Feature dim mismatch -> probe_x.shape={probe_x.shape}, gallery_x.shape={gallery_x.shape}. Skipping.")
                    continue

                # compute distance and guard topk
                try:
                    dist = cuda_dist(probe_x, gallery_x, metric)
                except Exception as e:
                    msg_mgr.log_error(f"    ❌ cuda_dist failed -> type={type_}, probe_view={probe_view}, gallery_view={gallery_view}, err={e}")
                    continue

                # defensive: ensure gallery size >= 1 and topk safe
                g_size = dist.shape[1] if len(dist.shape) > 1 else 0
                if g_size < 1:
                    msg_mgr.log_error(f"    ❌ gallery size zero -> type={type_}, probe_view={probe_view}, gallery_view={gallery_view}. Skipping.")
                    continue
                if g_size < 2:
                    msg_mgr.log_warning(f"    ⚠️ gallery size < 2 ({g_size}) -> using topk({g_size}) to avoid out-of-range.")
                    k = g_size
                else:
                    k = 2

                try:
                    idx = dist.topk(k, largest=False)[1].cpu().numpy()
                except Exception as e:
                    msg_mgr.log_error(f"    ❌ dist.topk failed -> type={type_}, probe_view={probe_view}, gallery_view={gallery_view}, err={e}")
                    continue

                # reshape probe labels and compute matches
                probe_labels_col = np.reshape(probe_y, [-1, 1])
                # To avoid indexing beyond available columns, use min(num_rank, k)
                used_rank = min(num_rank, k)
                try:
                    matches = probe_labels_col == gallery_y[idx[:, 0:used_rank]]
                    # Record misclassified IDs
                    misclassified = {}
                    for i in range(len(probe_y)):
                        if not matches[i, 0]:  # If the top-1 match is wrong
                            misclassified[probe_y[i]] = gallery_y[idx[i, 0]]
                    if misclassified:
                        msg_mgr.log_info(f"    ❌ Misclassified IDs -> {misclassified}")
                except Exception as e:
                    msg_mgr.log_error(f"    ❌ label indexing failed -> probe_count={len(probe_y)}, gallery_count={len(gallery_y)}, err={e}")
                    continue
                ...
                # if used_rank < num_rank, pad to keep shape consistent
                if used_rank < num_rank:
                    padded = np.zeros(num_rank)
                    padded[:used_rank] = accuracy_percentages
                    accuracy_percentages = padded

                acc[type_][v1, v2, :] = accuracy_percentages

            # --- compute mAP for this probe_view across all gallery angles ---
            ...
            if len(gallery_y_all_angle) == 0:
                msg_mgr.log_warning(f"    ⚠️ No gallery samples found for mAP -> type={type_}, probe_view={probe_view}. Setting MAP=0.")
                map[type_][v1] = 0
            else:
                try:
                    dist_all = cuda_dist(probe_x, gallery_x_all_angle, metric)
                    map_single_probe_view = compute_mAP(dist_all.cpu().numpy(), probe_y, gallery_y_all_angle, view[pseq_mask], view[gallery_all_angle_mask])
                    map[type_][v1] = map_single_probe_view
                except Exception as e:
                    msg_mgr.log_error(f"    ❌ compute_mAP failed -> type={type_}, probe_view={probe_view}, err={e}")
                    map[type_][v1] = 0
...
def single_view_gallery_evaluation(feature, label, seq_type, view, dataset, metric):
    ...
    for (type_, probe_seq) in probe_seq_dict[dataset].items():  # first is nm-03 nm-04
        ...
        for (v1, probe_view) in enumerate(view_list):   # 遍历 1-10 view 
            ...
            #print("probe_x shape (before):", probe_x.shape)
            #print("probe labels:", probe_y)
            ...
            for (v2, gallery_view) in enumerate(view_list):
                ...
                #print("gallery_x shape (before):", gallery_x.shape)
                #print("gallery labels:", gallery_y)
                
                dist = cuda_dist(probe_x, gallery_x, metric)
                idx = dist.topk(2, largest=False)[1].cpu().numpy()
