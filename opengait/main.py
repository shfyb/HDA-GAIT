...
def run_model(cfgs, training):
    ...
    print(f"Rank {rank} param num: {num_params}")
...
if __name__ == '__main__':
    print(f"🚀 Rank {os.environ.get('RANK', '?')} / Local Rank {os.environ.get('LOCAL_RANK', '?')} started")
    long_timeout = datetime.timedelta(seconds=3600)

    dist.init_process_group(
        backend='nccl', 
        init_method='env://', 
        timeout=long_timeout  # 在这里添加 timeout 参数
    )
