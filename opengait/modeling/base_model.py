...
from data.multi_domain_datasets import MultiDomainDataset
...
class BaseModel(MetaModel, nn.Module):
    ...
    def __init__(self, cfgs, training):
        ...
        if training:
            self.train_loader = self.get_loader_mutil(
    ...
    def get_loader_mutil(self, data_cfg, train=True):
        sampler_cfg = self.cfgs['trainer_cfg']['sampler'] if train else self.cfgs['evaluator_cfg']['sampler']
        
        # 数据加载
        dataset = MultiDomainDataset(data_cfg, train)
        ...
        # 使用多域collate函数
        from data.multi_domain_collate_fn import MultiDomainCollateFn
        
        loader = tordata.DataLoader(
            dataset=dataset,
            batch_sampler=sampler,
            collate_fn=MultiDomainCollateFn(dataset.label_set, dataset.types_set, sampler_cfg),
            num_workers=data_cfg['num_workers'])
    ...
    def _load_ckpt(self, save_name):
        ...
        # Downstream models may need an explicit, documented conversion when
        # the pretraining and fine-tuning modules are intentionally different
        # (for example, initializing a target-domain BN from the BN branch of
        # a pretrained IA-Norm).  The default path remains unchanged.
        preprocess_state_dict = getattr(
            self, 'preprocess_checkpoint_state_dict', None
        )
        if preprocess_state_dict is not None:
            model_state_dict = preprocess_state_dict(model_state_dict)
